...
// The browser host on NativeAOT-LLVM. Same engine, same CupriApp, same semantics as the Mono host
// (CupriFace.Web.Mono) -- what differs is the runtime underneath and therefore the interop: there
// is no Mono here, so no [JSImport]/[JSExport]. Exports are UnmanagedCallersOnly over the plain C
// ABI (ints, doubles, UTF-16 pointers) and imports are DllImports bound at link time against the
// Emscripten JS library in wwwroot/imports.js, which ships beside this file.
//
// Why it is a package: samples/WebLlvm used to BE this host, so a second app on this runtime had
// to copy ~740 lines of it, exactly as the Mono host's sample once did (#78, after #73).

/// <summary>The JS-facing surface: every export here is named by main.js/imports.js, which ship in
/// this same package so the halves cannot drift. Internal -- an app talks to <see cref="WebHost"/>.
/// Public because ILC exports it; nothing outside the package should call it.</summary>
