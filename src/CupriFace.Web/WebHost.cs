...
/// <summary>The JS-facing surface of the host. The names are the contract main.js binds to; it is
/// shipped in this package precisely so the two halves cannot drift apart.</summary>
public partial class WebHost
{
    ...
    /// <summary>What CupriWeb.Run was handed, before Init consumes it.</summary>
    ...
    /// which is after Main has run — so the app CupriWeb.Run registered is waiting here.</summary>
    [JSExport]
    internal static void Init()
    {
        _app = Pending ?? throw new InvalidOperationException(
            "No app was registered. A CupriFace.Web app's Main must call CupriWeb.Run(new MyApp()) — " +
        ...
        {
            using var fs = typeof(WebHost).Assembly.GetManifestResourceStream(res)!;
