# CupriFlash: which of it is the engine's?
...
and, more importantly, *look at* — as its own MCPSharp-style repository, **CupriFlash**.
...
engine's; if only a frame-by-frame renderer wants it, it is Flash's.** Applied to every element the
product needs:
...
| **Composition timeline** — `data-start` / `data-duration` / tracks, what is on stage at `t` | **CupriFlash** | Time as a *product* is a renderer's concern; a desktop window has no scene clock. App-level, over the engine's model/binding — no engine change |
| **Scripted interaction** — click at 1.2 s, type at 2.0 s | CupriFlash | Rides `DispatchClick` and the typing APIs; the engine already takes input without a window. Something a Chrome pipeline cannot do |
| The frame loop, PNG sequences, the ffmpeg pipe, alpha output | CupriFlash | ffmpeg never enters the engine |
| MCP server, config, tools, packaging, Docker, the agent skill | CupriFlash | House style, below |

Two things fall out of the table. Fonts are the one piece of *engine* work, and they are worth doing
first because every host benefits whether or not Flash ever ships. And nothing Flash needs is a
...
2. **Loading convenience.** `LoadFont(CupriSource)`, `LoadFonts(directory)`, and `CupriApp.Fonts`
   as the declarative list an app can enumerate; Flash's `--fonts` flag is one line over it.
...
5. **The strict policy.** `FontPolicy.RegisteredOnly`: a family that would resolve to the platform
   is an error naming the family, not a substitution. This is what Flash sets, and what makes a
...
6. **A report.** `doc.FontReport()` — every family the stylesheet asked for and which face answered
   (registered / platform / default). Flash's `lint` is this plus a verdict.
...
is the claim Flash will make, tested where Flash cannot test it.

---

## The product: CupriFlash

**The house style, taken from `GithubMCPSharp`:** a standalone Streamable-HTTP MCP server on
`ModelContextProtocol.AspNetCore` 1.3.0 / net10.0; `CupriFlash.json` with `Flash:` / `Server:` /
`Serilog:` sections, layered under `appsettings` and `.Local` files, env overrides under a
`CUPRIFLASH_` prefix, command line last; `Hosting/` (password middleware, console icon),
...
carry ffmpeg. The zips do not; `Flash:FfmpegPath` names it and `probe` says whether it is there.

The safety posture translates directly. A GitHub server is read-only by default with repository
allow-lists; a renderer *writes files*, so the analogue is `Flash:OutputRoot` (the only directory a
tool may write under), `Flash:CompositionRoots` (the only directories it may read compositions and
fonts from), `Flash:MaxFrames` / `MaxPixels` (a runaway request is a disk full), and
`Flash:EnableVideo` (off → no ffmpeg, PNG only).
...
A thin `cupriflash` CLI with the same verbs over the same services, because a CI job is not an MCP
client.
...
agents already know them, and the skills that teach them transfer. Flash's timeline layer keeps
...
steps are a second track: `data-flash-click="1.2"`, or a JSON sidecar of `(t, action)` pairs.
...
- **Cross-machine text.** HarfBuzz and Skia are pinned by the engine, so shaping and rasterising
  ...
  the fonts phase, before Flash promises anything.
...
| CupriFlash v1 — skeleton in house style, `render_frame`, `contact_sheet`, `render_frames`, `render_video`, `probe`, CLI, Docker with ffmpeg | 4–5 days (the render loop is the proof of concept) |
| timeline layer + `inspect` + `lint` | 3–4 days, after the keyframe-clock experiment |
| interaction track | 2 days |
| phase 2 — video seek in the engine, audio mux in Flash | not sized; deferred |
...
3. **CupriFlash, the repository**, house-style skeleton plus the frame tools and the video pipe —
...
- **The name.** "CupriFlash" says animation and carries Adobe's ghost; CupriMotion and CupriFrames
  are the safe alternatives. Cosmetic, and yours.
