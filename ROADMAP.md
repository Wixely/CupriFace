...
| Additional controls | 🟡 | **Hover tooltip + typeahead combobox + date picker done.** `<cupri-tooltip>` shows on `:hover`. `<cupri-combobox>` is a filtering typeahead. `<cupri-datepicker value="{{Date}}" open="{{Open}}">` is a date field (ISO `yyyy-MM-dd`) with a month-calendar popup — a day pick writes the value + closes; ‹ › page months in place (the new generic `data-set-keep` attribute sets a bound value without dismissing the overlay). Remaining candidates: time picker, sortable/virtualized data table; combobox/date-picker follow-up = arrow-key navigation. |
