...
| Additional controls | 🟡 | **Hover tooltip + typeahead combobox done.** `<cupri-tooltip>` shows on `:hover` (`open="true"` pins it). `<cupri-combobox value="{{X}}">` + `<cupri-option>`s: an editable single-line field with a suggestion list that filters as you type (free-text; dropdown reveals while focused via a `:focus` rule; picking a suggestion writes its value). Remaining candidates: date/time picker, sortable/virtualized data table; combobox follow-up = arrow-key highlight/selection. |
