...
| **Promote WebLlvm over WebWasm** | 🟡 | Cross-platform build is no longer the blocker (above). Remaining: the typing/clipboard path is exported but never exercised (17 `UnmanagedCallersOnly` entries incl. Copy/Cut/PasteText/Undo/Redo — "exported" is not "tested"), and the host still needs the `dotnet-experimental` preview feed, where WebWasm needs only the stock SDK + wasm workload. |
