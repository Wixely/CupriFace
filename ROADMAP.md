...
| **Video** | 🟢 | **Done (2026-08).** `<cupri-video src fit poster label controls autoplay muted loop>` — sources resolve exactly like images (embedded / disk / `data:` / policied https, one shared `SourceResolver`), sized like `<cupri-image>`, with engine-drawn accessible controls and OS/browser fullscreen via the `data-window-command` seam. Three tiers: the **engine** gained a generic live-surface lane (`data-cupri-surface` + `SurfaceRegistry`, reused later by 3D) that rides the image pipeline's own DrawImage/object-fit/damage path, plus `ClearHole` for host-composited surfaces; the **web host** lets the BROWSER decode into an underlaid `<video>` beneath a transparent hole (≈0 wasm bytes of codec, hardware decode, rect+clip synced per painted frame); the optional **`CupriFace.Media`** package plays WebM on desktop — managed EBML/Matroska demuxer (pure C#, unknown-size clusters and all three lacing modes) plus libvpx/libopus statically linked into ONE `cupricodecs` library per RID inside a single nupkg (the Skia model: build anywhere, run anywhere). Audio out rides the SDL binding the shell already ships — no new native dependency. Verified: real-codec decode + paint tests, a browser pass (play/pause/scroll-clip/fullscreen), the Showcase playing on desktop, and a CI gate that builds MediaProbe on Windows and decodes video from those binaries on Linux. Remaining: AV1 (dav1d), more RIDs than win-x64/linux-x64/osx-arm64, seek-bar UI, buffering/streaming sources. |
