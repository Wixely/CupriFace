...
| **Linux window path segfaults on a GPU-less/virtual X server** | 🔴 | **Diagnosed, not yet fixed.** gdb in CI puts the crash inside `gr_glinterface_assemble_interface` in libSkiaSharp — not GLFW and not the SDL fallback. A GPU-less/virtual X server hands back a GL *context object* whose entry points resolve but do nothing, and Skia's interface assembly dereferences what it gets back. Because it is a native SIGSEGV rather than an exception, `DesktopHost`'s fallback never runs. A first fix (probe `glGetString(GL_VERSION)` before handing the context to Skia, in `SkiaWindow.VerifyGlIsUsable`) did **not** stop it — either that call succeeds and Skia dies further along its assembly (it also calls `glGetIntegerv`/`glGetStringi`, and our loader returns a null pointer for anything unresolvable, which Skia does not check), or the crash precedes the probe. The guard is kept: it converts one real failure mode into a catchable exception and is verified not to disturb Windows. Evidence narrowing it: with the probe in place the app prints *nothing at all* before dying, so the guard never throws — `glGetString` succeeds and Skia dies later in its own assembly. **Workaround shipped:** `CUPRIFACE_SOFTWARE=1` skips the GL attempt entirely and uses the SDL software window, so affected users are unblocked. CI now gates on that path (which passes) and watches the GL path non-blockingly, so a permanently red build does not train everyone to ignore CI. Iterating costs a full CI round trip each time; the next step is a local Linux (WSL) rather than more blind attempts. |
