...
| **Linux window path segfaults on a GPU-less/virtual X server** | 🟢 | **Fixed (2026-08).** Diagnosed by running the real binary under gdb in CI, since no local Linux was available: the crash was inside `gr_glinterface_assemble_interface` in libSkiaSharp — not GLFW and not the SDL fallback, which is where the guesses had been. A GPU-less or virtual X server hands back a GL *context object* whose entry points resolve but do nothing, so `glGetString(GL_VERSION)` returns NULL and Skia parses a null pointer. Because that is a native SIGSEGV rather than an exception, `DesktopHost`'s existing fallback never got a chance to run. `SkiaWindow` now asks the context that same question first (via Silk's GL API, so no bespoke interop) and throws a catchable exception instead, which drops cleanly to the SDL software window. Regression-tested in CI: the Linux job launches the real executable under xvfb and fails if it dies. |
