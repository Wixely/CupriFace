...
///
/// <para><b>The drawing is now Khalkos3D's, and this file did not grow to accommodate it.</b> It used
/// to point at a hand-written renderer in <c>samples/Demo3d</c>; it now points at an engine, through
/// the same three-method <see cref="IGlContent"/> it always used. That the swap is one type name is
/// the strongest available evidence that the seam is a seam rather than a shape moulded around one
/// particular renderer.</para>
///
/// <para><c>samples/Demo3d</c> is deliberately left in place. It is the reference the engine was
/// measured against, and switching back is one line.</para>
/// </summary>
public static class Teapot3dSurface
{
    ...
    /// poster and a line of text saying so.
    /// </summary>
    public static GlViewport? TryAttach(CupriDocument doc, Action<string>? log = null)
    {
        ...
        return GlViewport.Attach(doc, "showcase3d", content, new GlViewportOptions
        {
            ...
            // The one thing CupriFace.Gl will not do for itself, and the reason it is a factory
            // rather than a built-in: making an offscreen context needs a windowing library, and
            // putting Silk.NET into the package would drag a desktop windowing stack into every
            // Android and browser build that referenced it. Desktop already has one, so desktop
            // supplies it — and gets a working viewport on a software window and in a headless
            // render, where there is no host GPU context to share.
